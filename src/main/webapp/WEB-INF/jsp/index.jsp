...
<body>
				...
				<div class="row">
					<div class="panel panel-default">
						<div class="panel-body">
							...
							<a class="pull-right" href="<%=path%>/main/blogList"> >>更多</a>
						</div>
					</div>
				</div>
			</div>
	...
	<c:if test="${pageNow > 1}">
		<a href="<%=path %>/main?pageNow=${pageNow-1}" id="up">上一页</a>
	</c:if>
	<c:forEach begin="1" end="${pageCount}" varStatus="page">
		<a href="<%=path %>/main?pageNow=${page.index}" class="yema">${page.index}</a>
	</c:forEach>
	<c:if test="${pageNow < pageCount}">
		<a href="<%=path %>/main?pageNow=${pageNow+1}" id="down">下一页</a>
	</c:if>
